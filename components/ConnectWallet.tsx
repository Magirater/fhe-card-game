...
export default function ConnectWallet({ onConnect, isConnecting, error }: ConnectWalletProps) {
  return (
    <div className="flex flex-col items-center justify-center text-center space-y-4 px-4 max-w-sm mx-auto">
      ...
      <div className="flex justify-center w-full">
        ...
        >
          {isConnecting ? (
            <>
              <div 
                className="border border-current border-t-transparent rounded-full animate-spin mr-1"
      ...
      {error && (
        <div className="mt-4 p-3 bg-red-900/20 border border-red-500/50 rounded-lg text-red-400 text-center text-sm max-w-xs mx-auto">
          {error}
        </div>
      )}
    </div>
